...
export async function POST(request: NextRequest) {
  ...
  if (file.type && !ALLOWED_MIME_TYPES.has(file.type)) {
    return NextResponse.json({ message: "Format favicon tidak didukung" }, { status: 400 });
  }
  ...
  const fileName = `favicons/${crypto.randomUUID()}.${extension}`;
  ...
  try {
    ...
    return NextResponse.json({ message: "Gagal mengunggah favicon" }, { status: 500 });
  }
}
